...
public class Solution7568 {
    public static void main(String[] args) throws IOException {
        ...
        List<Profile> people = new ArrayList<Profile>();

        for (int i = 0; i < n; i++) {
            Profile profile = new Profile();
            String[] line = bufferedReader.readLine().split(" ");
            profile.weight = Integer.parseInt(line[0]);
            profile.height = Integer.parseInt(line[1]);
            people.add(profile);
        }
        ...
        for (int i = 0; i < people.size(); i++) {
            bufferedWriter.write(people.get(i).rank + " ");
        }
...
class Profile {
    ...
    public Profile(int weight, int height) {
        this.weight = weight;
        this.height = height;
    }
}